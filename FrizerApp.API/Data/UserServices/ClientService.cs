...
namespace FrizerApp.API.Data.UserService
{
    public class ClientService : IClientService
    {
        ...
        public async Task<ServiceResponse> RegisterUserAsync(RegistrationModel model)
        {
            ...
            var registeredEntity = appDbContext.Registrations.Add(new RegistrationModel()
            {
                Name = model.Name!,
                Email = model.Email,
        ...
        public async Task<ServiceResponse> LoginUserAsync(LoginModel model)
        {
            ...
            if (checkIfPasswordMatch)
            {
                ...
                var getUserRole = await appDbContext.UserRoles.Where(_ => _.UserId == getUser.Id).FirstOrDefaultAsync();
